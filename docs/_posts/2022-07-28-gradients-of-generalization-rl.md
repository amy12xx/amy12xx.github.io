...
In our quest to create intelligent agents, where intelligence has been, so far, benchmarked by human cognitive abilities, one of the fundamental goals of Machine Learning is Generalization. To that end, extensive research has been carried out in defining Generalization [[4]](#references) [[5]](#references) [[6]](#references), and towards building generalizable agents and models [[7]](#references) [[8]](#references) [[9]](#references). In the large deep learning models era, studying Generalization has taken on a new focus. Similarly, in Reinforcement Learning, several studies of the problem [[10]](#references) [[11]](#references) [[12]](#references) have been conducted, new benchmarks proposed [[13]](#references) [[14]](#references) [[15]](#references), and algorithms developed.
...
PPO is used as the learning algorithm, trained over 5 random seeds, and each policy is evaluated over 100 episodes. A CNN network is used to learn from visual observation data. To learn a latent space based on the original distances, I use NMDS as proposed in the original paper, on the generalization data, with Euclidean distance in the resultant latent space to compute the generalization curves. Implementation is done using Stable Baselines 3 [[30]](#references) and Scikit-Learn [[31]](#references).
...
In the case of orientation experiments, we find that the graph decays more gradually. This implies, that the agent is able to generalize better over the different orientations. This may be due to the limited variation in the orientation stimulus, or due to the spatial invariances using a CNN network. Using other distance metrics such as absolute cosine distances does not recover an exponential curve. Given that some orientations (less occluded) ought to be more easily learnable than others, I am uncertain if a stretched exponential can be recovered.
